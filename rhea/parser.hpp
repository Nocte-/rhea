...
struct constraint_grammar
    : boost::spirit::qi::grammar<iterator, constraint()> //,
                                 //boost::spirit::ascii::space_type>
{
    typedef boost::spirit::ascii::space_type    space_type;

    constraint_grammar(var_map& vars) : constraint_grammar::base_type(constr)
    {
        ...
        constr =
              lineq   [ _val = phx::construct<constraint>(_1) ]
            | linineq [ _val = phx::construct<constraint>(_1) ]
            ;

        lineq =
              (expr >> '=' >> expr) [ _val = phx::construct<linear_equation>(_1, _2) ]
            ;

        linineq =
              (expr >> "<=" >> expr)[ _val = phx::construct<linear_inequality>(_1, relation::leq, _2) ]
            | (expr >> ">=" >> expr)[ _val = phx::construct<linear_inequality>(_1, relation::geq, _2) ]
            ;

        expr =
              double_               [ _val = _1 ]
            //| lexeme[raw[(ascii::alpha >> *(ascii::alnum | '_'))]] [ vars.add, _val = vars[_1] ]
            | vars                  [ _val = _1 ]
